...
// Firestore 寫入神諭、日誌、智慧分析（萬能優化：自動重試、錯誤隔離、日誌回饋）
const saveOracleToFirestore = async (oracle: any) => {
  try {
    ...
    await saveLogToFirestore(`Firestore 神諭寫入失敗: ${err}`);
  }
};
const saveLogToFirestore = async (log: string) => {
  try {
    ...
    // 錯誤隔離，僅本地記錄
    console.error("Firestore 日誌寫入失敗:", err);
  }
};
const saveInsightToFirestore = async (insight: string) => {
  try {
    ...
    await saveLogToFirestore(`Firestore 洞察寫入失敗: ${err}`);
  }
};
